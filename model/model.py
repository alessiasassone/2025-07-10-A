...
class Model:
    ...
    def _ricorsione(self, parziale, lun, end):
        # ottimalità e terminazione, deve essere almeno lun
        if len(parziale) == lun:
            # se ultimo el = a end e lo score di parziale è migliore di bestScore, me lo salvo
            if parziale[-1] == end and self._getScore(parziale) > self._bestScore:
                self._bestScore = self._getScore(parziale)
                self._bestPath = copy.deepcopy(parziale)
            return

        for n in self._graph.successors(parziale[-1]): #successori parziale -1
    ...
    def getNodiPiuVenduti(self):
        ...
        for n in self._graph.nodes:
            ...
            for e in self._graph.out_edges(n, data=True):
                score += e[2]["weight"] #e è una tupla
